"""NER extraction RPC handler.

Implements the ExtractEntities gRPC method: three-stage pipeline
(GLINER + Flair → ensemble merge → LLM verification).
"""
...
from ner_service.pipeline.ensemble import merge as merge_entities
from ner_service.llm import reviewer as llm_reviewer
...
def _build_proto_entity(me) -> ner_pb2.Entity:
...
def _build_proto_entity_from_dict(d: dict) -> ner_pb2.Entity:
    ...
    action = d.get("action", LLM_ACTION_CONFIRM)
    if action == LLM_ACTION_CORRECT:
        status = ner_pb2.ENTITY_STATUS_LLM_CORRECTED
    elif action == LLM_ACTION_ADD:
...
class NerServiceHandler(ner_pb2_grpc.NerServiceServicer):
    ...
    def ExtractEntities(self, request: ner_pb2.ExtractRequest, context: grpc.ServicerContext) -> ner_pb2.ExtractResponse:
        ...
        min_conf = request.min_confidence or MIN_CONFIDENCE
        max_entities = request.max_entities or MAX_ENTITIES
        ...
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=EXTRACTION_TIMEOUT)
            if t.is_alive():
        ...
        merged, conflict_count = merge_entities(gliner_results, flair_results, min_conf)
        ...
        if llm_reviewer.should_review(
            ...
            if reviewed and reviewed != candidates:
                ...
                reviewed = validate_llm_output(reviewed, request.text, candidate_spans)
