...
from ner_service.config import ENABLE_LLM, LLM_MAX_RETRIES, LLM_TIMEOUT_SECONDS, OLLAMA_HOST, OLLAMA_MODEL, get_logger
from ner_service.config import FLAIR_TAG_MAP, LLM_ACTION_ADD, LLM_ACTION_CONFIRM, LLM_ACTION_CORRECT, LLM_ACTION_REJECT
...
def review(
    ...
    Args:
        ...
        timeout: HTTP timeout in seconds. Defaults to LLM_TIMEOUT_SECONDS.
...
def apply_review(
    ...
    for item in reviewed:
        ...
        if not context and source_text:
            context = extract_context(source_text, name)

        if action == LLM_ACTION_ADD:
            final.append({
                "text": name,
                "type": item.get("type", ""),
                "confidence": float(item.get("confidence", 0.8)),
                "context": context,
                "action": LLM_ACTION_ADD,
                "reasoning": item.get("reasoning", ""),
            })
        elif action == LLM_ACTION_CORRECT:
            final.append({
                "text": name,
                "type": item.get("type", ""),
                "confidence": float(item.get("confidence", 0.8)),
                "context": context,
                "action": LLM_ACTION_CORRECT,
                "reasoning": item.get("reasoning", ""),
            })
        elif action == LLM_ACTION_CONFIRM:
            final.append({
                "text": name,
                "type": item.get("type", ""),
                "confidence": float(item.get("confidence", 0.8)),
                "context": context,
                "action": LLM_ACTION_CONFIRM,
                "reasoning": item.get("reasoning", ""),
            })
